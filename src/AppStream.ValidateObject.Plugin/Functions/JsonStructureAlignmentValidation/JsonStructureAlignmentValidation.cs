...
public class JsonStructureAlignmentValidation
{
    ...
    [OpenApiParameter(name: "json1", Description = "First escaped JSON object to verify", Type = typeof(string), In = ParameterLocation.Query, Required = true)]
    [OpenApiParameter(name: "json2", Description = "Second escaped JSON object that is a sample JSON to verify against", Type = typeof(string), In = ParameterLocation.Query, Required = true)]
